<?php
	 ...
	 * listing a collection of Proveedor objects.  It includes
	 ...
	 * 
	 */
	class ProveedorDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Proveedor datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Proveedor object, or it can be any QQNode extending from QQN::Proveedor().
		 * 
		 ...
		 * sorting using two fields from the Proveedor object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Proveedor
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdPROVEEDOR=<?=urlencode($_ITEM->IdPROVEEDOR)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdPROVEEDOR)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdPROVEEDOR, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Proveedor::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Proveedor, given the clauses above
			$this->DataSource = Proveedor::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Proveedor-based QQNode.
		 * It will also verify that it is a proper Proveedor-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'PROVEEDOR') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "PROVEEDOR".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdPROVEEDOR': return QQN::Proveedor()->IdPROVEEDOR;
				case 'Nombre': return QQN::Proveedor()->Nombre;
				case 'Direccion': return QQN::Proveedor()->Direccion;
				case 'Telefono': return QQN::Proveedor()->Telefono;
				case 'Email': return QQN::Proveedor()->Email;
				case 'PAISIdPAIS': return QQN::Proveedor()->PAISIdPAIS;
				case 'PAISIdPAISObject': return QQN::Proveedor()->PAISIdPAISObject;
				default: throw new QCallerException('Simple Property not found in ProveedorDataGrid content: ' . $mixContent);
