<?php
	 ...
	 * listing a collection of CodigoPago objects.  It includes
	 ...
	 * 
	 */
	class CodigoPagoDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this CodigoPago datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CodigoPago object, or it can be any QQNode extending from QQN::CodigoPago().
		 * 
		 ...
		 * sorting using two fields from the CodigoPago object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CodigoPago
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intLICENCIAIdLICENCIA=<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>&intTIPODEPAGOIdTIPODEPAGO=<?=urlencode($_ITEM->TIPODEPAGOIdTIPODEPAGO)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>/<?=urlencode($_ITEM->TIPODEPAGOIdTIPODEPAGO)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->LICENCIAIdLICENCIA . "," . $_ITEM->TIPODEPAGOIdTIPODEPAGO, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = CodigoPago::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from CodigoPago, given the clauses above
			$this->DataSource = CodigoPago::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CodigoPago-based QQNode.
		 * It will also verify that it is a proper CodigoPago-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'CODIGO_PAGO') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "CODIGO_PAGO".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'LICENCIAIdLICENCIA': return QQN::CodigoPago()->LICENCIAIdLICENCIA;
