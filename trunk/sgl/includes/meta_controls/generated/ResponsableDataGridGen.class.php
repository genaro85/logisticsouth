<?php
	 ...
	 * listing a collection of Responsable objects.  It includes
	 ...
	 * 
	 */
	class ResponsableDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Responsable datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Responsable object, or it can be any QQNode extending from QQN::Responsable().
		 * 
		 ...
		 * sorting using two fields from the Responsable object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Responsable
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intEMPLEADOIdEMPLEADO=<?=urlencode($_ITEM->EMPLEADOIdEMPLEADO)?>&intLICENCIAIdLICENCIA=<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->EMPLEADOIdEMPLEADO)?>/<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->EMPLEADOIdEMPLEADO . "," . $_ITEM->LICENCIAIdLICENCIA, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Responsable::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Responsable, given the clauses above
			$this->DataSource = Responsable::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Responsable-based QQNode.
		 * It will also verify that it is a proper Responsable-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'RESPONSABLE') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "RESPONSABLE".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'EMPLEADOIdEMPLEADO': return QQN::Responsable()->EMPLEADOIdEMPLEADO;
				case 'EMPLEADOIdEMPLEADOObject': return QQN::Responsable()->EMPLEADOIdEMPLEADOObject;
				case 'LICENCIAIdLICENCIA': return QQN::Responsable()->LICENCIAIdLICENCIA;
				case 'LICENCIAIdLICENCIAObject': return QQN::Responsable()->LICENCIAIdLICENCIAObject;
				case 'FechaInicio': return QQN::Responsable()->FechaInicio;
				case 'FechaFin': return QQN::Responsable()->FechaFin;
				default: throw new QCallerException('Simple Property not found in ResponsableDataGrid content: ' . $mixContent);
