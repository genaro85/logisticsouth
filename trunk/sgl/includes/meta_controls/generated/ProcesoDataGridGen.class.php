<?php
	 ...
	 * listing a collection of Proceso objects.  It includes
	 ...
	 * 
	 */
	class ProcesoDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Proceso datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Proceso object, or it can be any QQNode extending from QQN::Proceso().
		 * 
		 ...
		 * sorting using two fields from the Proceso object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Proceso
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdPROCESO=<?=urlencode($_ITEM->IdPROCESO)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdPROCESO)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdPROCESO, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Proceso::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Proceso, given the clauses above
			$this->DataSource = Proceso::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Proceso-based QQNode.
		 * It will also verify that it is a proper Proceso-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'PROCESO') {
