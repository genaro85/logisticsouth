<?php
	 ...
	 * listing a collection of Administrador objects.  It includes
	 ...
	 * 
	 */
	class AdministradorDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Administrador datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Administrador object, or it can be any QQNode extending from QQN::Administrador().
		 * 
		 ...
		 * sorting using two fields from the Administrador object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Administrador
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdADMINISTRADOR=<?=urlencode($_ITEM->IdADMINISTRADOR)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdADMINISTRADOR)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdADMINISTRADOR, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Administrador::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Administrador, given the clauses above
			$this->DataSource = Administrador::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Administrador-based QQNode.
		 * It will also verify that it is a proper Administrador-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ADMINISTRADOR') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ADMINISTRADOR".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdADMINISTRADOR': return QQN::Administrador()->IdADMINISTRADOR;
				case 'Nombre': return QQN::Administrador()->Nombre;
				case 'Apellido': return QQN::Administrador()->Apellido;
