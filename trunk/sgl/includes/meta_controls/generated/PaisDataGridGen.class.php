<?php
	 ...
	 * listing a collection of Pais objects.  It includes
	 ...
	 * 
	 */
	class PaisDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Pais datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Pais object, or it can be any QQNode extending from QQN::Pais().
		 * 
		 ...
		 * sorting using two fields from the Pais object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Pais
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdPAIS=<?=urlencode($_ITEM->IdPAIS)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdPAIS)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdPAIS, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Pais::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Pais, given the clauses above
			$this->DataSource = Pais::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Pais-based QQNode.
		 * It will also verify that it is a proper Pais-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'PAIS') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "PAIS".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdPAIS': return QQN::Pais()->IdPAIS;
				case 'Nombre': return QQN::Pais()->Nombre;
				default: throw new QCallerException('Simple Property not found in PaisDataGrid content: ' . $mixContent);
