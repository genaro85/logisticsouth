<?php
	 ...
	 * listing a collection of ListaDeDocumento objects.  It includes
	 ...
	 * 
	 */
	class ListaDeDocumentoDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this ListaDeDocumento datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ListaDeDocumento object, or it can be any QQNode extending from QQN::ListaDeDocumento().
		 * 
		 ...
		 * sorting using two fields from the ListaDeDocumento object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ListaDeDocumento
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intDOCUMENTOIdDOCUMENTO=<?=urlencode($_ITEM->DOCUMENTOIdDOCUMENTO)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->DOCUMENTOIdDOCUMENTO)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->DOCUMENTOIdDOCUMENTO, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = ListaDeDocumento::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from ListaDeDocumento, given the clauses above
			$this->DataSource = ListaDeDocumento::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ListaDeDocumento-based QQNode.
		 * It will also verify that it is a proper ListaDeDocumento-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'LISTA_DE_DOCUMENTO') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "LISTA_DE_DOCUMENTO".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'DOCUMENTOIdDOCUMENTO': return QQN::ListaDeDocumento()->DOCUMENTOIdDOCUMENTO;
				case 'DOCUMENTOIdDOCUMENTOObject': return QQN::ListaDeDocumento()->DOCUMENTOIdDOCUMENTOObject;
				case 'PROCESOIdPROCESO': return QQN::ListaDeDocumento()->PROCESOIdPROCESO;
				case 'PROCESOIdPROCESOObject': return QQN::ListaDeDocumento()->PROCESOIdPROCESOObject;
				case 'FASEIdFASE': return QQN::ListaDeDocumento()->FASEIdFASE;
				case 'FASEIdFASEObject': return QQN::ListaDeDocumento()->FASEIdFASEObject;
				default: throw new QCallerException('Simple Property not found in ListaDeDocumentoDataGrid content: ' . $mixContent);
