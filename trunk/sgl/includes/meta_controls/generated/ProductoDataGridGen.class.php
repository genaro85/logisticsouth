 ...
 * listing a collection of Producto objects.  It includes
 ...
 *
 */
class ProductoDataGridGen extends QDataGrid {
     ...
     * way of adding a column to this Producto datagrid.  The description of a column's
     * content can be either a text string description of a simple field name
     * in the Producto object, or it can be any QQNode extending from QQN::Producto().
     *
     ...
     * sorting using two fields from the Producto object.
     *
     ...
     *
     * @param mixed $mixContent string or QQNode from Producto
    ...
    public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
        switch ($intArgumentType) {
            case QMetaControlArgumentType::QueryString:
                $strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdPRODUCTO=<?=urlencode($_ITEM->IdPRODUCTO)?>';
                break;
            case QMetaControlArgumentType::PathInfo:
                $strLinkUrl .= '/<?=urlencode($_ITEM->IdPRODUCTO)?>';
                break;
            default:
    ...
    public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
        $strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdPRODUCTO, false); ?>>' . $strLinkHtml . '</a>';
        $colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
        $this->AddColumn($colEditColumn);
        return $colEditColumn;
    }
    ...
    public function MetaDataBinder() {
        ...
        if ($this->Paginator) {
            $this->TotalItemCount = Producto::QueryCount($objConditions);
        }
        ...
        // Set the DataSource to be a Query result from Producto, given the clauses above
        $this->DataSource = Producto::QueryArray($objConditions, $objClauses);
    }
     ...
     *
     * Given a QQNode or a Text String, this will return a Producto-based QQNode.
     * It will also verify that it is a proper Producto-based QQNode, and will throw an exception otherwise.
     *
    ...
    protected function ResolveContentItem($mixContent) {
        if ($mixContent instanceof QQNode) {
            ...
            if ($mixContent->_RootTableName == 'PRODUCTO') {
                ...
                throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "PRODUCTO".');
        } else if (is_string($mixContent)) switch ($mixContent) {
                case 'IdPRODUCTO': return QQN::Producto()->IdPRODUCTO;
                case 'CodigoArancelario': return QQN::Producto()->CodigoArancelario;
                case 'Descripcion': return QQN::Producto()->Descripcion;
                case 'NumeroRegSanitario': return QQN::Producto()->NumeroRegSanitario;
                case 'Volumen': return QQN::Producto()->Volumen;
                case 'Unidad': return QQN::Producto()->Unidad;
                default: throw new QCallerException('Simple Property not found in ProductoDataGrid content: ' . $mixContent);
