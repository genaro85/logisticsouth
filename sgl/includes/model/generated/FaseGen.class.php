<?php
	 ...
	 * @property-read Fase $_FaseAsFASEIdFASE the value for the private _objFaseAsFASEIdFASE (Read-Only) if set due to an expansion on the FASE.FASE_idFASE reverse relationship
	 * @property-read Fase[] $_FaseAsFASEIdFASEArray the value for the private _objFaseAsFASEIdFASEArray (Read-Only) if set due to an ExpandAsArray on the FASE.FASE_idFASE reverse relationship
	...
	class FaseGen extends QBaseClass implements IteratorAggregate {
		...
		/**
		 * Protected member variable that maps to the database column FASE.FASE_idFASE
		...
		/**
		 * Private member variable that stores a reference to a single FaseAsFASEIdFASE object
		 * (of type Fase), if this Fase object was restored with
		 * an expansion on the FASE association table.
		 * @var Fase _objFaseAsFASEIdFASE;
		 */
		private $_objFaseAsFASEIdFASE;

		/**
		 * Private member variable that stores a reference to an array of FaseAsFASEIdFASE objects
		 * (of type Fase[]), if this Fase object was restored with
		 * an ExpandAsArray on the FASE association table.
		 * @var Fase[] _objFaseAsFASEIdFASEArray;
		 */
		private $_objFaseAsFASEIdFASEArray = array();
		...
		/**
		 * Protected member variable that contains the object pointed by the reference
		 * in the database column FASE.FASE_idFASE.
		 *
		...
		public function Initialize()
		{
			...
			$this->intFASEIdFASE = Fase::FASEIdFASEDefault;
		}
		...
		public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $strExpandAsArrayNodes = null, $arrPreviousItems = null, $strColumnAliasArray = array()) {
			...
			if (($strExpandAsArrayNodes) && is_array($arrPreviousItems) && count($arrPreviousItems)) {
				foreach ($arrPreviousItems as $objPreviousItem) {            
					if ($objPreviousItem->intIdFASE == $objDbRow->GetColumn($strAliasName, 'Integer')) {        
						...
						// Expanding reverse references: FaseAsFASEIdFASE
						$strAlias = $strAliasPrefix . 'faseasfaseidfase__idFASE';
						$strAliasName = array_key_exists($strAlias, $strColumnAliasArray) ? $strColumnAliasArray[$strAlias] : $strAlias;
						if ((array_key_exists($strAlias, $strExpandAsArrayNodes)) &&
							(!is_null($objDbRow->GetColumn($strAliasName)))) {
							if ($intPreviousChildItemCount = count($objPreviousItem->_objFaseAsFASEIdFASEArray)) {
								$objPreviousChildItems = $objPreviousItem->_objFaseAsFASEIdFASEArray;
								$objChildItem = Fase::InstantiateDbRow($objDbRow, $strAliasPrefix . 'faseasfaseidfase__', $strExpandAsArrayNodes, $objPreviousChildItems, $strColumnAliasArray);
								if ($objChildItem) {
									$objPreviousItem->_objFaseAsFASEIdFASEArray[] = $objChildItem;
								}
							} else {
								$objPreviousItem->_objFaseAsFASEIdFASEArray[] = Fase::InstantiateDbRow($objDbRow, $strAliasPrefix . 'faseasfaseidfase__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
							}
							$blnExpandedViaArray = true;
						}
			...
			if (isset($arrPreviousItems) && is_array($arrPreviousItems)) {
				foreach ($arrPreviousItems as $objPreviousItem) {
					...
					if (array_diff($objPreviousItem->_objFaseAsFASEIdFASEArray, $objToReturn->_objFaseAsFASEIdFASEArray) != null) {
						continue;
					}
			...
			// Check for FaseAsFASEIdFASE Virtual Binding
			$strAlias = $strAliasPrefix . 'faseasfaseidfase__idFASE';
			$strAliasName = array_key_exists($strAlias, $strColumnAliasArray) ? $strColumnAliasArray[$strAlias] : $strAlias;
			if (!is_null($objDbRow->GetColumn($strAliasName))) {
				if (($strExpandAsArrayNodes) && (array_key_exists($strAlias, $strExpandAsArrayNodes)))
					$objToReturn->_objFaseAsFASEIdFASEArray[] = Fase::InstantiateDbRow($objDbRow, $strAliasPrefix . 'faseasfaseidfase__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
				else
					$objToReturn->_objFaseAsFASEIdFASE = Fase::InstantiateDbRow($objDbRow, $strAliasPrefix . 'faseasfaseidfase__', $strExpandAsArrayNodes, null, $strColumnAliasArray);
			}
		...
		*/
		public static function LoadArrayByFASEIdFASE($intFASEIdFASE, $objOptionalClauses = null) {
			// Call Fase::QueryArray to perform the LoadArrayByFASEIdFASE query
			try {
				return Fase::QueryArray(
					QQ::Equal(QQN::Fase()->FASEIdFASE, $intFASEIdFASE),
					$objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
		...
		*/
		public static function CountByFASEIdFASE($intFASEIdFASE) {
			// Call Fase::QueryCount to perform the CountByFASEIdFASE query
			return Fase::QueryCount(
				QQ::Equal(QQN::Fase()->FASEIdFASE, $intFASEIdFASE)
			);
		}
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case '_FaseAsFASEIdFASE':
					/**
					 * Gets the value for the private _objFaseAsFASEIdFASE (Read-Only)
					 * if set due to an expansion on the FASE.FASE_idFASE reverse relationship
					 * @return Fase
					 */
					return $this->_objFaseAsFASEIdFASE;

				case '_FaseAsFASEIdFASEArray':
					/**
					 * Gets the value for the private _objFaseAsFASEIdFASEArray (Read-Only)
					 * if set due to an ExpandAsArray on the FASE.FASE_idFASE reverse relationship
					 * @return Fase[]
					 */
					return (array) $this->_objFaseAsFASEIdFASEArray;
				...
				case 'FASEIdFASEObject':
					...
					if (is_null($mixValue)) {
						...
						if (is_null($mixValue->IdFASE))
							throw new QCallerException('Unable to set an unsaved FASEIdFASEObject for this Fase');
		...
		*/ 
		...
		// Related Objects' Methods for FaseAsFASEIdFASE
		//-------------------------------------------------------------------

		/**
		 * Gets all associated FasesAsFASEIdFASE as an array of Fase objects
		 * @param QQClause[] $objOptionalClauses additional optional QQClause objects for this query
		 * @return Fase[]
		*/ 
		public function GetFaseAsFASEIdFASEArray($objOptionalClauses = null) {
			if ((is_null($this->intIdFASE)))
				return array();

			try {
				return Fase::LoadArrayByFASEIdFASE($this->intIdFASE, $objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
		...
		/**
		 * Counts all associated FasesAsFASEIdFASE
		 * @return int
		*/ 
		public function CountFasesAsFASEIdFASE() {
			if ((is_null($this->intIdFASE)))
				return 0;

			return Fase::CountByFASEIdFASE($this->intIdFASE);
		}

		/**
		 * Associates a FaseAsFASEIdFASE
		 * @param Fase $objFase
		 * @return void
		*/ 
		public function AssociateFaseAsFASEIdFASE(Fase $objFase) {
			if ((is_null($this->intIdFASE)))
				throw new QUndefinedPrimaryKeyException('Unable to call AssociateFaseAsFASEIdFASE on this unsaved Fase.');
			if ((is_null($objFase->IdFASE)))
				throw new QUndefinedPrimaryKeyException('Unable to call AssociateFaseAsFASEIdFASE on this Fase with an unsaved Fase.');
		...
		/**
		 * Unassociates a FaseAsFASEIdFASE
		 * @param Fase $objFase
		 * @return void
		*/ 
		public function UnassociateFaseAsFASEIdFASE(Fase $objFase) {
			if ((is_null($this->intIdFASE)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociateFaseAsFASEIdFASE on this unsaved Fase.');
			if ((is_null($objFase->IdFASE)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociateFaseAsFASEIdFASE on this Fase with an unsaved Fase.');
		...
		/**
		 * Unassociates all FasesAsFASEIdFASE
		 * @return void
		*/ 
		public function UnassociateAllFasesAsFASEIdFASE() {
			if ((is_null($this->intIdFASE)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociateFaseAsFASEIdFASE on this unsaved Fase.');
		...
		/**
		 * Deletes an associated FaseAsFASEIdFASE
		 * @param Fase $objFase
		 * @return void
		*/ 
		public function DeleteAssociatedFaseAsFASEIdFASE(Fase $objFase) {
			if ((is_null($this->intIdFASE)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociateFaseAsFASEIdFASE on this unsaved Fase.');
			if ((is_null($objFase->IdFASE)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociateFaseAsFASEIdFASE on this Fase with an unsaved Fase.');
		...
		/**
		 * Deletes all associated FasesAsFASEIdFASE
		 * @return void
		*/ 
		public function DeleteAllFasesAsFASEIdFASE() {
			if ((is_null($this->intIdFASE)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociateFaseAsFASEIdFASE on this unsaved Fase.');
     ...
     * @property-read QQReverseReferenceNodeFase $FaseAsFASEIdFASE
	...
	class QQNodeFase extends QQNode {
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'FaseAsFASEIdFASE':
					return new QQReverseReferenceNodeFase($this, 'faseasfaseidfase', 'reverse_reference', 'FASE_idFASE');
