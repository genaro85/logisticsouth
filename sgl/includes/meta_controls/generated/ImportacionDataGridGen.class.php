<?php
	 ...
	 * listing a collection of Importacion objects.  It includes
	 ...
	 * 
	 */
	class ImportacionDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Importacion datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Importacion object, or it can be any QQNode extending from QQN::Importacion().
		 * 
		 ...
		 * sorting using two fields from the Importacion object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Importacion
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intTRANSPORTEIdTRANSPORTE=<?=urlencode($_ITEM->TRANSPORTEIdTRANSPORTE)?>&intLICENCIAIdLICENCIA=<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->TRANSPORTEIdTRANSPORTE)?>/<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->TRANSPORTEIdTRANSPORTE . "," . $_ITEM->LICENCIAIdLICENCIA, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Importacion::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Importacion, given the clauses above
			$this->DataSource = Importacion::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Importacion-based QQNode.
		 * It will also verify that it is a proper Importacion-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'IMPORTACION') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "IMPORTACION".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'TRANSPORTEIdTRANSPORTE': return QQN::Importacion()->TRANSPORTEIdTRANSPORTE;
				case 'TRANSPORTEIdTRANSPORTEObject': return QQN::Importacion()->TRANSPORTEIdTRANSPORTEObject;
				case 'LICENCIAIdLICENCIA': return QQN::Importacion()->LICENCIAIdLICENCIA;
				case 'LICENCIAIdLICENCIAObject': return QQN::Importacion()->LICENCIAIdLICENCIAObject;
				case 'FechaDeSalida': return QQN::Importacion()->FechaDeSalida;
				case 'FechaLlegada': return QQN::Importacion()->FechaLlegada;
				case 'Tipo': return QQN::Importacion()->Tipo;
				case 'PAISOrigen': return QQN::Importacion()->PAISOrigen;
				case 'PAISOrigenObject': return QQN::Importacion()->PAISOrigenObject;
				case 'PAISDestino': return QQN::Importacion()->PAISDestino;
				case 'PAISDestinoObject': return QQN::Importacion()->PAISDestinoObject;
				default: throw new QCallerException('Simple Property not found in ImportacionDataGrid content: ' . $mixContent);
