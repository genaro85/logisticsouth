<?php
	 ...
	 * listing a collection of Transporte objects.  It includes
	 ...
	 * 
	 */
	class TransporteDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Transporte datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Transporte object, or it can be any QQNode extending from QQN::Transporte().
		 * 
		 ...
		 * sorting using two fields from the Transporte object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Transporte
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdTRANSPORTE=<?=urlencode($_ITEM->IdTRANSPORTE)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdTRANSPORTE)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdTRANSPORTE, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Transporte::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Transporte, given the clauses above
			$this->DataSource = Transporte::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Transporte-based QQNode.
		 * It will also verify that it is a proper Transporte-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'TRANSPORTE') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "TRANSPORTE".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdTRANSPORTE': return QQN::Transporte()->IdTRANSPORTE;
				case 'Nombre': return QQN::Transporte()->Nombre;
				case 'Direccion': return QQN::Transporte()->Direccion;
				case 'Telefono': return QQN::Transporte()->Telefono;
				case 'PAISIdPAIS': return QQN::Transporte()->PAISIdPAIS;
				case 'PAISIdPAISObject': return QQN::Transporte()->PAISIdPAISObject;
				default: throw new QCallerException('Simple Property not found in TransporteDataGrid content: ' . $mixContent);
