<?php
	 ...
	 * listing a collection of Fase objects.  It includes
	 ...
	 * 
	 */
	class FaseDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Fase datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Fase object, or it can be any QQNode extending from QQN::Fase().
		 * 
		 ...
		 * sorting using two fields from the Fase object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Fase
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdFASE=<?=urlencode($_ITEM->IdFASE)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdFASE)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdFASE, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Fase::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Fase, given the clauses above
			$this->DataSource = Fase::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Fase-based QQNode.
		 * It will also verify that it is a proper Fase-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'FASE') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "FASE".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdFASE': return QQN::Fase()->IdFASE;
				...
				case 'FASEIdFASE': return QQN::Fase()->FASEIdFASE;
				case 'FASEIdFASEObject': return QQN::Fase()->FASEIdFASEObject;
