<?php
	 ...
	 * listing a collection of TipoDePago objects.  It includes
	 ...
	 * 
	 */
	class TipoDePagoDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this TipoDePago datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the TipoDePago object, or it can be any QQNode extending from QQN::TipoDePago().
		 * 
		 ...
		 * sorting using two fields from the TipoDePago object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from TipoDePago
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdTIPODEPAGO=<?=urlencode($_ITEM->IdTIPODEPAGO)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdTIPODEPAGO)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdTIPODEPAGO, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = TipoDePago::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from TipoDePago, given the clauses above
			$this->DataSource = TipoDePago::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a TipoDePago-based QQNode.
		 * It will also verify that it is a proper TipoDePago-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'TIPO_DE_PAGO') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "TIPO_DE_PAGO".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdTIPODEPAGO': return QQN::TipoDePago()->IdTIPODEPAGO;
				case 'Nombre': return QQN::TipoDePago()->Nombre;
				default: throw new QCallerException('Simple Property not found in TipoDePagoDataGrid content: ' . $mixContent);
