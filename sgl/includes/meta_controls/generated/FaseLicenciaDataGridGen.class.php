<?php
	 ...
	 * listing a collection of FaseLicencia objects.  It includes
	 ...
	 * 
	 */
	class FaseLicenciaDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this FaseLicencia datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the FaseLicencia object, or it can be any QQNode extending from QQN::FaseLicencia().
		 * 
		 ...
		 * sorting using two fields from the FaseLicencia object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from FaseLicencia
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intLICENCIAIdLICENCIA=<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>&intFASEIdFASE=<?=urlencode($_ITEM->FASEIdFASE)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>/<?=urlencode($_ITEM->FASEIdFASE)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->LICENCIAIdLICENCIA . "," . $_ITEM->FASEIdFASE, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = FaseLicencia::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from FaseLicencia, given the clauses above
			$this->DataSource = FaseLicencia::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a FaseLicencia-based QQNode.
		 * It will also verify that it is a proper FaseLicencia-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'FASE_LICENCIA') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "FASE_LICENCIA".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'LICENCIAIdLICENCIA': return QQN::FaseLicencia()->LICENCIAIdLICENCIA;
				case 'LICENCIAIdLICENCIAObject': return QQN::FaseLicencia()->LICENCIAIdLICENCIAObject;
				case 'FASEFechaInicio': return QQN::FaseLicencia()->FASEFechaInicio;
				case 'FASEFechaFin': return QQN::FaseLicencia()->FASEFechaFin;
				case 'FASEIdFASE': return QQN::FaseLicencia()->FASEIdFASE;
				case 'FASEIdFASEObject': return QQN::FaseLicencia()->FASEIdFASEObject;
				default: throw new QCallerException('Simple Property not found in FaseLicenciaDataGrid content: ' . $mixContent);
