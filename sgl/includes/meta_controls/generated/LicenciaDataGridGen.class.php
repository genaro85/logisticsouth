<?php
	 ...
	 * listing a collection of Licencia objects.  It includes
	 ...
	 * 
	 */
	class LicenciaDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Licencia datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Licencia object, or it can be any QQNode extending from QQN::Licencia().
		 * 
		 ...
		 * sorting using two fields from the Licencia object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Licencia
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdLICENCIA=<?=urlencode($_ITEM->IdLICENCIA)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdLICENCIA)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdLICENCIA, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Licencia::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Licencia, given the clauses above
			$this->DataSource = Licencia::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Licencia-based QQNode.
		 * It will also verify that it is a proper Licencia-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'LICENCIA') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "LICENCIA".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdLICENCIA': return QQN::Licencia()->IdLICENCIA;
				case 'EMPRESAIdEMPRESA': return QQN::Licencia()->EMPRESAIdEMPRESA;
				case 'EMPRESAIdEMPRESAObject': return QQN::Licencia()->EMPRESAIdEMPRESAObject;
				case 'PROVEEDORIdPROVEEDOR': return QQN::Licencia()->PROVEEDORIdPROVEEDOR;
				case 'PROVEEDORIdPROVEEDORObject': return QQN::Licencia()->PROVEEDORIdPROVEEDORObject;
				case 'FechaInicio': return QQN::Licencia()->FechaInicio;
				case 'FechaFin': return QQN::Licencia()->FechaFin;
				case 'FechaFinEstimada': return QQN::Licencia()->FechaFinEstimada;
				case 'NumeroProforma': return QQN::Licencia()->NumeroProforma;
				case 'NumeroCNP': return QQN::Licencia()->NumeroCNP;
				case 'VencimientoCNP': return QQN::Licencia()->VencimientoCNP;
				case 'Status': return QQN::Licencia()->Status;
				case 'FormaPago': return QQN::Licencia()->FormaPago;
				case 'Tipo': return QQN::Licencia()->Tipo;
				case 'Flete': return QQN::Licencia()->Flete;
				case 'Seguro': return QQN::Licencia()->Seguro;
				case 'PROCESOIdPROCESO': return QQN::Licencia()->PROCESOIdPROCESO;
				case 'PROCESOIdPROCESOObject': return QQN::Licencia()->PROCESOIdPROCESOObject;
				default: throw new QCallerException('Simple Property not found in LicenciaDataGrid content: ' . $mixContent);
