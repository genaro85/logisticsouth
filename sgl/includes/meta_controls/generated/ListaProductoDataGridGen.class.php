<?php
	 ...
	 * listing a collection of ListaProducto objects.  It includes
	 ...
	 * 
	 */
	class ListaProductoDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this ListaProducto datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ListaProducto object, or it can be any QQNode extending from QQN::ListaProducto().
		 * 
		 ...
		 * sorting using two fields from the ListaProducto object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ListaProducto
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intLICENCIAIdLICENCIA=<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>&intPRODUCTOIdPRODUCTO=<?=urlencode($_ITEM->PRODUCTOIdPRODUCTO)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>/<?=urlencode($_ITEM->PRODUCTOIdPRODUCTO)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->LICENCIAIdLICENCIA . "," . $_ITEM->PRODUCTOIdPRODUCTO, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			// Set the DataSource to be a Query result from ListaProducto, given the clauses above
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ListaProducto-based QQNode.
		 * It will also verify that it is a proper ListaProducto-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'LISTA_PRODUCTO') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "LISTA_PRODUCTO".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'LICENCIAIdLICENCIA': return QQN::ListaProducto()->LICENCIAIdLICENCIA;
				case 'LICENCIAIdLICENCIAObject': return QQN::ListaProducto()->LICENCIAIdLICENCIAObject;
				case 'PRODUCTOIdPRODUCTO': return QQN::ListaProducto()->PRODUCTOIdPRODUCTO;
				case 'PRODUCTOIdPRODUCTOObject': return QQN::ListaProducto()->PRODUCTOIdPRODUCTOObject;
				case 'PRODUCTOCantidad': return QQN::ListaProducto()->PRODUCTOCantidad;
				case 'PRODUCTOVolumen': return QQN::ListaProducto()->PRODUCTOVolumen;
				case 'PRODUCTOUnidad': return QQN::ListaProducto()->PRODUCTOUnidad;
				case 'PRODUCTOCostoUnitario': return QQN::ListaProducto()->PRODUCTOCostoUnitario;
				default: throw new QCallerException('Simple Property not found in ListaProductoDataGrid content: ' . $mixContent);
