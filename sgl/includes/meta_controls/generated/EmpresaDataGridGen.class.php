<?php
	 ...
	 * listing a collection of Empresa objects.  It includes
	 ...
	 * 
	 */
	class EmpresaDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Empresa datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Empresa object, or it can be any QQNode extending from QQN::Empresa().
		 * 
		 ...
		 * sorting using two fields from the Empresa object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Empresa
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdEMPRESA=<?=urlencode($_ITEM->IdEMPRESA)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdEMPRESA)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdEMPRESA, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Empresa::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Empresa, given the clauses above
			$this->DataSource = Empresa::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Empresa-based QQNode.
		 * It will also verify that it is a proper Empresa-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'EMPRESA') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "EMPRESA".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdEMPRESA': return QQN::Empresa()->IdEMPRESA;
				case 'Nombre': return QQN::Empresa()->Nombre;
				case 'Rif': return QQN::Empresa()->Rif;
				case 'Direccion': return QQN::Empresa()->Direccion;
				case 'Telefono': return QQN::Empresa()->Telefono;
				case 'Email': return QQN::Empresa()->Email;
				case 'Login': return QQN::Empresa()->Login;
				case 'Password': return QQN::Empresa()->Password;
				default: throw new QCallerException('Simple Property not found in EmpresaDataGrid content: ' . $mixContent);
