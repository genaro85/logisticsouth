<?php
	 ...
	 * listing a collection of EtapaLicencia objects.  It includes
	 ...
	 * 
	 */
	class EtapaLicenciaDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this EtapaLicencia datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the EtapaLicencia object, or it can be any QQNode extending from QQN::EtapaLicencia().
		 * 
		 ...
		 * sorting using two fields from the EtapaLicencia object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from EtapaLicencia
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intLICENCIAIdLICENCIA=<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>&intETAPAIdETAPA=<?=urlencode($_ITEM->ETAPAIdETAPA)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->LICENCIAIdLICENCIA)?>/<?=urlencode($_ITEM->ETAPAIdETAPA)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->LICENCIAIdLICENCIA . "," . $_ITEM->ETAPAIdETAPA, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = EtapaLicencia::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from EtapaLicencia, given the clauses above
			$this->DataSource = EtapaLicencia::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a EtapaLicencia-based QQNode.
		 * It will also verify that it is a proper EtapaLicencia-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ETAPA_LICENCIA') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ETAPA_LICENCIA".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'LICENCIAIdLICENCIA': return QQN::EtapaLicencia()->LICENCIAIdLICENCIA;
				case 'LICENCIAIdLICENCIAObject': return QQN::EtapaLicencia()->LICENCIAIdLICENCIAObject;
				case 'ETAPAIdETAPA': return QQN::EtapaLicencia()->ETAPAIdETAPA;
				case 'ETAPAIdETAPAObject': return QQN::EtapaLicencia()->ETAPAIdETAPAObject;
				case 'ETAPAFechaInicio': return QQN::EtapaLicencia()->ETAPAFechaInicio;
				case 'ETAPAFechaFin': return QQN::EtapaLicencia()->ETAPAFechaFin;
				default: throw new QCallerException('Simple Property not found in EtapaLicenciaDataGrid content: ' . $mixContent);
