<?php
	 ...
	 * listing a collection of Empleado objects.  It includes
	 ...
	 * 
	 */
	class EmpleadoDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Empleado datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Empleado object, or it can be any QQNode extending from QQN::Empleado().
		 * 
		 ...
		 * sorting using two fields from the Empleado object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Empleado
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdEMPLEADO=<?=urlencode($_ITEM->IdEMPLEADO)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdEMPLEADO)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdEMPLEADO, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Empleado::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Empleado, given the clauses above
			$this->DataSource = Empleado::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Empleado-based QQNode.
		 * It will also verify that it is a proper Empleado-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'EMPLEADO') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "EMPLEADO".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'IdEMPLEADO': return QQN::Empleado()->IdEMPLEADO;
				case 'Nombre': return QQN::Empleado()->Nombre;
				case 'Apellido': return QQN::Empleado()->Apellido;
