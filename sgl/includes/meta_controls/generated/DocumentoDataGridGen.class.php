<?php
	 ...
	 * listing a collection of Documento objects.  It includes
	 ...
	 * 
	 */
	class DocumentoDataGridGen extends QDataGrid {
		 ...
		 * way of adding a column to this Documento datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Documento object, or it can be any QQNode extending from QQN::Documento().
		 * 
		 ...
		 * sorting using two fields from the Documento object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Documento
		...
		public function MetaAddEditLinkColumn($strLinkUrl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit', $intArgumentType = QMetaControlArgumentType::PathInfo) {
			switch ($intArgumentType) {
				case QMetaControlArgumentType::QueryString:
					$strLinkUrl .= (strpos($strLinkUrl, '?') !== false ? '&' : '?').'intIdDOCUMENTO=<?=urlencode($_ITEM->IdDOCUMENTO)?>';
					break;
				case QMetaControlArgumentType::PathInfo:
					$strLinkUrl .= '/<?=urlencode($_ITEM->IdDOCUMENTO)?>';
					break;
				default:
		...
		public function MetaAddEditProxyColumn(QControlProxy $pxyControl, $strLinkHtml = 'Edit', $strColumnTitle = 'Edit') {
			$strHtml = '<a href="#" <?= $_FORM->GetControl("' . $pxyControl->ControlId . '")->RenderAsEvents($_ITEM->IdDOCUMENTO, false); ?>>' . $strLinkHtml . '</a>';
			$colEditColumn = new QDataGridColumn($strColumnTitle, $strHtml, 'HtmlEntities=False');
			$this->AddColumn($colEditColumn);
			return $colEditColumn;
		}
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Documento::QueryCount($objConditions);
			}
			...
			// Set the DataSource to be a Query result from Documento, given the clauses above
			$this->DataSource = Documento::QueryArray($objConditions, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Documento-based QQNode.
		 * It will also verify that it is a proper Documento-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'DOCUMENTO') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "DOCUMENTO".');
				...
				case 'ListaDeDocumento': return QQN::Documento()->ListaDeDocumento;
